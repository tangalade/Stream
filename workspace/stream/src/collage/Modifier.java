...
public class Modifier extends IPacketCodableInputStream {
  private int maxBufferSize = -1;
  private ModifierWorkerThread workerThread = null;
  
  private IPacketCodableInputStream pStream = null;
  ...
  public Modifier(IPacketCodableInputStream pStream, 
      IStreamCoder aOEncoder, IStreamCoder vOEncoder, 
      IStreamCoder aODecoder, IStreamCoder vODecoder, 
      int maxBufferSize) {
    this.pStream = pStream;
    this.aOEncoder = aOEncoder;
    this.vOEncoder = vOEncoder;
    this.aODecoder = aODecoder;
    this.vODecoder = vODecoder;
  ...
  public Modifier(IPacketCodableInputStream pStream, 
      IStreamCoder aOEncoder, IStreamCoder vOEncoder,
      IStreamCoder aODecoder, IStreamCoder vODecoder) {
    this(pStream, aOEncoder, vOEncoder, aODecoder, vODecoder, 100);
  }  
  ...
  private void setupStreams() {
    ...
    for (int i=0; i<numStreams; i++) {
      ...
      if (cType == ICodec.Type.CODEC_TYPE_AUDIO) {
        ...
        System.out.println("SampleFormat: " + ic.getSampleFormat());;
        ...
        if (cType == ICodec.Type.CODEC_TYPE_AUDIO)
        {
          while (offset < iPacket.getSize()) {
            ...
            while (numSamplesConsumed < outSamples.getNumSamples()) {
              ...
              System.out.println("Modifier input packet: " + iPacket);
              System.out.println("Modifier decoded audio: " + inSamples);
              System.out.println("Modifier encoded audio: " + outSamples);
              if (oPacket.isComplete()) {
                oPacket.setStreamIndex(iPacket.getStreamIndex());
                System.out.println("Modifier output packet: " + oPacket);
                try {
                  packets.put(oPacket);
          ...
          while (offset < iPacket.getSize()) {
            ...
            if (inFrame.isComplete()) {
              ...
              System.out.println("Modifier encode video: " + outFrame);
